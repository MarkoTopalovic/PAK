...
IMPLICIT NONE
!
INTEGER :: count1, count2, i, j, l, jid, k, m
!
...
REAL(kind=real_acc)    :: Volj, deltasig(mcm_ndim),rhoi
real(kind=real_acc), dimension(3,3) :: sigmai, qi, sigmaj, qj
!
! Set default values
!
othird = 1.0_d/3.0_d
...
!
if(mcm_contacttype.eq.1) then ! vignjeviceva kontakt repulzivna sila
...
enddo
!
else ! kernel repulzija za razlicite cestice Topalovic
do i = mcm_svp, mcm_evp
   !
   par(i)%repulsion(1:mcm_ndim) = 0.0
   if(par(i)%ncont.gt.0) then
   ...
   sigmai = par(i)%sigma
   qi = par(i)%q
      do k=1,par(i)%ncont + par(i)%g_ncont
      call mcm_get_j_kernelcont_info(i,k,xj,massj,rhoj,hj,kj,sigmaj,qj)
      !
      ...
      !
      do l=1,mcm_ndim
        par(i)%repulsion(l) = par(i)%repulsion(l) + massj * deltasig(l)
      enddo
      !
     enddo  
      !
