...
else if (mcm_istate.lt.1000) then
	write(unit=extension,fmt=1020) mcm_istate	
else if (mcm_istate.lt.10000) then
	write(unit=extension,fmt=1030) mcm_istate		
else
	write(unit=extension,fmt=1040) mcm_istate
end if
1000 format(I1,'.vtk')
1010 format(I2,'.vtk')
1020 format(I3,'.vtk')
...
WRITE(88,*)"VECTORS Normals double"

DO i = 1,mcm_np1
 WRITE(88,882) par(i)%bndnorm(1),par(i)%bndnorm(2),par(i)%bndnorm(3)
ENDDO

WRITE(88,*)"VECTORS Repulsion double"

DO i = 1,mcm_np1
 WRITE(88,882) par(i)%repulsion(1),par(i)%repulsion(2),par(i)%repulsion(3)
ENDDO

WRITE(88,*)"VECTORS ContactForce double"

DO i = 1,mcm_np1
 WRITE(88,882) par(i)%contactforce(1),par(i)%contactforce(2),par(i)%contactforce(3)
ENDDO

WRITE(88,*)"VECTORS NormalForce double"

DO i = 1,mcm_np1
 WRITE(88,882) par(i)%normalforce(1),par(i)%normalforce(2),par(i)%normalforce(3)
ENDDO

WRITE(88,*)"VECTORS Acceleration double"
...
883 FORMAT ("CELLS ",3hNumb," ",3hNumb)
 ...
 900 FORMAT ("SCALARS LifeStatus double 1")
...
DO i = 1,mcm_np1
if (par(i).mat.eq.matID) then
 WRITE(88,882) CVORNESILE(INODEBROJAC,1),CVORNESILE(INODEBROJAC,2),CVORNESILE(INODEBROJAC,3)
...
883 FORMAT ("CELLS ",3hNumb," ",3hNumb)
 ...
 900 FORMAT (I5)
 901 FORMAT ("SCALARS ContactZ double 1")

!
end subroutine WriteVTKmat

! Tijana added - subroutine for printing results to a vtk file

SUBROUTINE mcm_write_vtk
...
IF (mcm_timestep.LT.10) THEN
 mcm_filename_vtk = mcm_fileout(1:mcm_filelen(2))//"-"//str1(1:1)//".vtk"
ELSE IF (mcm_timestep.LT.100) THEN
 mcm_filename_vtk = mcm_fileout(1:mcm_filelen(2))//"-"//str1(1:2)//".vtk"
ELSE IF (mcm_timestep.LT.1000) THEN
 mcm_filename_vtk = mcm_fileout(1:mcm_filelen(2))//"-"//str1(1:3)//".vtk"
ELSE IF (mcm_timestep.LT.10000) THEN
 mcm_filename_vtk = mcm_fileout(1:mcm_filelen(2))//"-"//str1(1:4)//".vtk"
ENDIF 

!mcm_filename_vtk = mcm_fileout(1:mcm_filelen(2))//"-"//str//".vtk"
!
   open(unit=88,file=mcm_filename_vtk,status='unknown',form='formatted')
!_________________________
!
...
CLOSE(UNIT=88)
  ...
  84 FORMAT ("DATASET UNSTRUCTURED_GRID")
  ! ovde update-ovati broj cvorova !!!
 881 FORMAT ("POINTS 6 double")
 882 FORMAT (F15.8,F15.8,F15.8)
 ! ovde update-ovati broj cvorova !!!
 883 FORMAT ("CELLS 6 12")
 884 FORMAT ("1 ",I6)
 885 FORMAT ("CELL_TYPES ",I5)
 886 FORMAT ("1")
 887 FORMAT ("POINT_DATA ",I5)
 ...
 896 FORMAT ("SCALARS Shear_Stress_XZ double 1")
 897 FORMAT ("SCALARS Shear_Stress_YZ double 1")
 898 FORMAT ("SCALARS Shear_Stress_YZ double 1")

end SUBROUTINE mcm_write_vtk