...
enddo
DO I=1,NP
         par(mcm_np-NP+I)%dispbc = 7 ! fem cestice su fiksirane
         par(mcm_np-NP+I)%x(1) = VTKCVOROVI(I,1)
         par(mcm_np-NP+I)%x(2) = VTKCVOROVI(I,2)
         par(mcm_np-NP+I)%x(3) = VTKCVOROVI(I,3)
         par(mcm_np-NP+I)%mat = par(mcm_np-NP)%mat+1
         !POSLEDNJA REGULARNE CESTICA IMA MAT odredjeni materijal a fem cestice imaju id materijala uvecan za 1
ENDDO
!
