...
!
real(kind=real_acc) :: mcm_endtime, mcm_tssf, mcm_itss, mcm_oldtime
...
REAL(kind=real_acc) :: b11, b12, b13, b21, b22, b23, b31, b32, b33 ! rotation matrix - 25-04-02 - Tom - mat52 mat53
  real(kind=real_acc), dimension(:,:), allocatable :: averagematnormal
!
!----------------------------------------------------------------------------
! Base accelerations
!
...
!  integer :: n_symcont                              ! number of symmetry contact neighbours
  ...
  real(kind=real_acc) :: bndnormlength              ! Length of unit normal vector to surface for boundary particles

  real(kind=real_acc), dimension(3) :: repulsion	! contact acceleration
  real(kind=real_acc), dimension(3) :: contactforce	!
  real(kind=real_acc), dimension(3) :: normalforce	! contact acceleration
  real(kind=real_acc), dimension(3) :: kernelrepulsion	! calculataed for friction
  ! tensors
...
contains
 ...
 !
   ...
   allocate (averagematnormal(mcm_nummat,3), stat=ierr)
  if(ierr.ne.0) then
   write(*,'("Allocation error for average normals")')
   stop
  endif
...
MODULE CVOROVI
    LOGICAL ALLOCIRANICVOROVI 
    LOGICAL ALLOCIRANIELEMENTI
    LOGICAL ALLOCIRANAPOMERANJA
    INTEGER*4 NPcvorova
    INTEGER*4 ipaks_filelen
    REAL*8, DIMENSION(:,:), ALLOCATABLE :: VTKCVOROVI
    REAL*8, DIMENSION(:,:), ALLOCATABLE :: VTKPOMERANJA
END MODULE

MODULE ZFLUID
    INTEGER*4 NPTI
    LOGICAL ALLOCIRANECVORNESILE
    integer, dimension(:), allocatable :: NODEBROJ
    REAL*8, DIMENSION(:,:), ALLOCATABLE :: CVORNESILE
