...
CE       DIMENSION GUSM(99,*) WILL BE CHANGED INTO GUSM(100,*)
...
CE          MODPRO(NMODM): NEEDED ARRAY PER ONE INTEGRATING POINT
    5       IBTC=IBTC+1
            ...
            IF(NMODM.EQ.49)THEN ! UBACENA METODA KOJA REGULISE KOEFICIJENT TERMICKOG SIRENJA I POCETNU TEMPERATURU ZA MICUNOV MODEL
                LFUN=MREPER(1)
            LNTA=MREPER(2)
            LTEM=MREPER(3)
            MATE=MREPER(4)
            CALL MEL349(A(LFUN),A(LNTA),A(LTEM),MATE,TGT)
                ENDIF
C
...
C
         IF (NMODM.EQ.3.OR.NMODM.EQ.4.OR.NMODM.EQ.49)THEN
             CALL MAMOD3(STRAIN,TGT) ! ovde ulazi kad racuna deformacije za micunov model
         ENDIF
C
C
CS       RACUNANJE NAPONA
