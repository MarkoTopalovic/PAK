C$DEBUG      
C=========================================================================
      SUBROUTINE VRACUN(TT1,SILE,NEL,ID,NZAD,ZADVRE,NGPSIL,
...
C=======================================================================
      ...
      CALL VPREB(TT21,TT1,ID,NEL,NBREL)
      CALL VCLEARD(TT210,NDIM)
      CALL VPREB(TT210,TT10,ID,NEL,NBREL)
...
C INTEGRACIJA U GAUSOVIM TACKAMA
      ...
      DO 170 J=1,IBRGT
      NGAUS=NGAUS+1
      CALL VINTERP(R(IBRGT,I),S(IBRGT,J),0,0)
...
C      if(nbrel.eq.1) CALL Vwrr(h,9,'h   ')
C      if(nbrel.eq.1) CALL Vwrr(zvhx,9,'zvhx')
C      if(nbrel.eq.1) CALL Vwrr(zvhy,9,'zvhy')
  170 CONTINUE
  180 CONTINUE
...
C
     ...
     1(NODE1.EQ.N4 .AND. NODE2.EQ.N1)) THEN
      CALL VINTERP(1.D0,S(NUMGAU,I),1,1)
       GOTO 220
      ENDIF 
...
C
     ...
     1(NODE1.EQ.N2 .AND. NODE2.EQ.N1)) THEN
      CALL VINTERP(R(NUMGAU,I),1.D0,2,1)
       GOTO 220
      ENDIF 
...
C        ENDIF
      ...
      ENDDO
C
       CALL VPSKEFN(SKEF,SKEFN)
       CALL VSPAKDE (SILE,F36,LM2,NDES)

	IF (ISOLVER.EQ.0) THEN
        IF (MAXCUR.GT.0) THEN
          CALL VSPAKUJ(A(LSK),MAXA,SKEFN,LM2,NDES)
        ELSE
...
C         CALL VSPARSE_ELEM_LEFT(SKEF,NEL,ID,NDES,NBREL,NDIM,1)
        ...
        ENDIF
      ENDIF
c      if(nbrel.eq.1) CALL Vwrr(skefn,ndes*(ndes+1)/2,'ske ')
c      if(nbrel.eq.167) CALL Vwrr(f36,ndes,'f36 ')
C=======================================================================
...
C==========================================================================
	...
	IF (MAXCUR.GT.0) THEN
         ...
         ELSE
          if(myid.eq.0) CALL VSPARS_ZADLEV(stiff_n,noneros,
...
C
      IF (NKONT.GT.0) THEN
        CALL VRKONTUR(TT1,ID,KONT,NEL,AKONST,CORD)
      ENDIF
C
...
C INDIKATOR NJUTN-KOTESOVE INTEGRACIJE (0-NE,1-DA)
	...
	IF(KOTES.EQ.1)IBRKT=3
      ...
      NGAUS=NGAUS+1
	IF(KOTES.EQ.1) THEN
         CALL VINTERP(RK(IBRKT,I),SK(IBRKT,J),0,0)
         WDT=WK(IBRKT,I)*WK(IBRKT,J)*DETJ*DEBLJ
	ELSE
         CALL VINTERP(R(IBRKT,I),S(IBRKT,J),0,0)
...
C POTPROGRAM ZA RACUNANJE PROTOKA PO KONTURI:
       CALL VPRKONT1(TT1,ID,UBRZ,CORD,NZAD,IOSA,IVECT,ZADVRE,KOJK)
C========================================================================

C RACUNANJE PROTOKA DUZ KONTURE UNUTAR MREZE
C
      IF (NKONT.GT.0) THEN
        CALL VRKONTV(VECTJ,KONT,NEL,CORD,VG)
      ENDIF
C
      WRITE(IIZLAZ,*)'PERIOD NUMBER ',NNPER         
      IF (KKORAK.GT.0) THEN
      CALL VIZLLST(ID,TT1,KKORAK,VECTJ,VVREME,FZAPR,VG,GG,NEL,
     1            KOTES)
      CALL VSTAGP1(TT1,ID,NASLOV,VVREME,KKORAK,1,NPT,18,0,NET,NEL,
...
C
      ...
      CALL VSTAU09(VECTJ,CORD,ID,NPT,49,41,2,KKORAK,NZAD,
     1 NUMZAD,KONT,TT1)
      CALL VSTAU09(GRADJN,CORD,ID,NPT,49,51,2,KKORAK,NZAD,NUMZAD,KONT,
     1            TT1)
      CALL VSTAU09(FZAPR,CORD,ID,NPT,49,61,2,KKORAK,NZAD,
     1 NUMZAD,KONT,TT1)
      ENDIF
  ...
  600 CONTINUE
     ...
     +   CALL VIZBACI(TT1,CORD,ID,NPT,NEL,NET,IOSA,NDIM,IIZLAZ)
      RETURN      
      END
C=======================================================================
C=======================================================================

      SUBROUTINE VIZLLST(ID,TT1,KKORAK,VECTJ,VVREME,FZAPR,VG,GG,
     1                  NEL,KOTES)
      USE KONTURE
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      COMMON /ULAZNI/ IULAZ,IIZLAZ,IPAKT
...
C
C=========================================================================
      SUBROUTINE VRKONTUR(TT1,ID,KONT,NEL,AKONST,CORD)
      USE KONTURE
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C
      COMMON /TRENUT/ TT21,H,HP,ZVHX,ZVHY,HV2,HV3,ZVXT,ZVYT,DETJ,
...
C
C=========================================================================
      SUBROUTINE VRKONTV(VECTJ,KONT,NEL,CORD,VG)
      USE KONTURE
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C
      COMMON /STAC/ NSTAC,NSLP,IBROJ
